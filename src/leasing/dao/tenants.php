...
class tenants extends _dao {
  ...
  function getCurrentTenants(int $property_id = 0) {
    ...
    $where[] = $_w = 'p.`forrent` = 1';
    $where_autoextend[] = $_w;
    $where_noleggio[] = $_w;

    $where[] = $_w = sprintf(
      '(o.`lease_start_inaugural` <= %s OR o.`lease_start` <= %s)',
    ...
    $where[] = sprintf(
      '( COALESCE( o.`vacate`, %s) = %s OR o.`vacate` > %s)',
    ...
    // just on where, not autoextend
    $where[] = sprintf(
      '(o.lease_term = %s OR COALESCE( o.`lease_end`, %s) = %s OR DATE(o.`lease_end`) > %s)',
      $this->quote('Periodic agreement'),
    ...
    $sqlTemplate =
      ...
      FROM
        `offer_to_lease` o
          LEFT JOIN
        `properties` p ON p.`id` = o.`property_id`
      WHERE
        %s
      ORDER BY
        o.`property_id` ASC,
    ...
    if ($res = $this->Result($sql)) {
      ...
      if ($property_id) {
        /**
         * search single property and not found, check noleggio
         */
        if (!$_dtoSet) {
          /**
        ...
        /**
         * get all the tenants from noleggio
         */
        // sys::logSQL(sprintf('<%s> %s', $sqlNoleggio, __METHOD__));
        if ($res = $this->Result($sqlNoleggio)) {
          while ($_dto = $res->dto()) {
